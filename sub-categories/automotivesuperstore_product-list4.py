...
baseurl = 'https://automotivesuperstore.com.au/service/vehicle-service-kits/holden-service-kits'
        ...
        for linkCategoryProduct in linkCategoryProducts:
            if linkCategoryProduct['href'] not in processed_urls:
                ...
                Automotivesuperstore = {
                    'Category (Parent)': 'Home ' + ' Servicing ' ,
                    'Category URL (Parent)': '/'.join(baseurl.split('/')[:3]),
                    'Category - Leaf (Child 1)': 'Home ' + ' Servicing ' + ' ' + baseurl.split('/')[-2].replace('-', ' ').title() + '  ' + baseurl.split('/')[-1].replace('-', ' ').title(),
                    'Category URL - Leaf (Child 1)': url_Category_URL_Leaf_Child1,
                    'Product URL': linkCategoryProduct['href'],
                    'PartNumber': PartNumber,
                    'Product Title': productTitle, 
                    'Product Subtitle': 'None',
